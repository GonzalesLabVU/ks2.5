function create_sparse_neuron_from_ks_adc(sessions, ks_working_directory, adc_input_directory, neuron_output_directory, varargin)
...
for i_session = 1:numel(sessions)
    ...
    oe    = loadOE(session);
    %  Load ADC events
    adc_event_file = dir(fullfile(adc_input_directory, ['*', session.subject_identifier, '*', num2str(session.session_number), '*adc_event*']));
    ...
    task_counter = 0;
        ...
        %   Add spikes to AllData
            for i_beh = session.beh_order % Loop through chronologically ordered behavior files
                ...
                AllData_c = add_spikes_sparse_adc(AllData, sp, oe, event_structure.analog_events, task_counter);
                toc
                %
